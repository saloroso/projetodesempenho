...
// -- SCRIPT PARA FAZER LOUGOT - TRAZER USUÁRIO PADRÃO A ZERO --------------------- // 
    function Salvar_Logout(){
        var usuariologado = "";               
        Salvar_Usuariologado(usuariologado);  
    }

     function Salvar_Logout2(){
        var usuariologado = "";               
        Salvar_Usuariologado(usuariologado); 
    }
 ...
 // -- FUNÇÃO PARA CRIAR RESPONDENTE DE UM LIDER --------------------- // 
    function Criar_membro(nome, codigo, desempenho){
          this.nome = nome;
          this.codigo = codigo;
          ...
          this.media_geral = 0;
          this.respondido = 0; 
          this.ranking = "Indefinido"; 
       alert ("Respondente criado com sucesso.");
    }

  // -- VERIFICAR SE O ADM JÁ ESTÁ CADASTRADO NO BANCO DE DADOS --------------------- // 
     function Verifica_Adm(nome1,email1,senha1,lista){ 
          alert ("Entrei na função verificar dados de cadastro do adm.");
          var lista2 = lista;     
  ...
  // -- VERIFICAR SE O MEMBRO JÁ ESTA NA LISTA DE MEMBROS DO ADMINISTRADOR --------------------- // 
      function Verificar_Membro(codigo2, lista){
          alert ("Entrei na função verificar membro.");
          var lista2 = lista;
          var lista2 = sessionStorage.getItem("BANCO_ADMS");
          lista2 = JSON.parse(lista2);  
          var usuario = sessionStorage.getItem("USUARIO_LOGADO");
          usuario = JSON.parse(usuario);  
          var tamanho = lista2.length;  
          alert ("Quantidade de administradores até agora: " + tamanho);
          alert ("Usuário é: " + usuario);
          for (i=0;i<tamanho;i++){
              if (lista[i].nome == usuario) {
                  var tamanho4 = lista[i].membros.length; 
                  for (j=0;j<tamanho4;j++){
                      if ((codigo2 == "") || (lista[i].membros[j].codigo == codigo2) {
                      alert ("Entrei na função para ver se o código está errado e ele está vazio ou errado.");
                      return false;
                      }
                  }
              }
          alert ("Codigo não existe e pode cadastrar.");
          return true;
      }
  ...
  // -- FUNÇÃO PARA CADASTRAR ADM NO BANCO DE DADOS --------------------- // 
    function Cadastrar_Adm(){   
          alert ("Entrei na função cadastrar adm.");
          ...
          if (verificar==true){
               ...
               var valor = "#basicExampleModal3";
               document.getElementById("Disparador_Modal").setAttribute("data-target", valor);  
          } 
    }
    
    // FUNÇÃO CHAMADA PARA CADASTRAR MEMBRO ---------------------- //
    function Cadastrar_Membro(lista2){
        alert ("Entrei na função cadastrar membro.");
        var lista = lista2;
        var lista = sessionStorage.getItem("BANCO_ADMS");
        lista = JSON.parse(lista);  
        var nome2 = document.getElementById("nome2").value;
        var desempenho2 = document.getElementById("desempenho2").value;
        var codigo2 = document.getElementById("codigo2").value;  
        alert ("Nome: " + nome2);
        alert ("Desempenho: " + desempenho2);
        alert ("Código: " + codigo2);
        var verificar2 = Verificar_Membro(codigo2, lista);
        if (verificar2==true){
           alert ("Entrei no if de criar membro.");
           var novo_membro = new Criar_membro(nome2, codigo2, desempenho2);
           var tamanho_banco = listadeadms.length; 
           var tamanho_equipe = 0;
           for (i=0;i<tamanho_banco;i++){
               if (listadeadms[i].nome==usuariologado){
                listadeadms[i].membros.push(novo_membro);
                   tamanho_equipe = listadeadms[i].membros.length;
                   alert ("Novo tamanho da equipe é: " + tamanho_equipe);
                   document.body.innerHTML = document.body.innerHTML.replace('Substituir3', tamanho_equipe);
                   Salvar_Bancodeadms(lista);
                   alert ("Membro cadastrado.");
               } else {
                   alert ("Membro não cadastrado.");
               }
           }
        }
    }
  ...
  // -- FUNÇÃO PARA LOGAR NO BANCO DE DADOS --------------------- // 
   ...
   // -- FUNÇÃO PARA RECEBER VALORES DO FORMULÁRIO E CALCULAR --------------------- // 
    function Processar_Formulario(lista1){   
        alert ("Entrou na função Processar_Formulario 10.");     
        var lista = lista1;  
        var lista = sessionStorage.getItem("BANCO_ADMS");
        lista = JSON.parse(lista);       
        ...
        if ((auto1<0) || (auto1>100) || (auto2<0) || (auto2>100) || (auto3<0) || (auto3>100)
            ...
            || (ini1<0) || (ini1>100) || (ini2<0) || (ini2>100) || (ini3<0) || (ini3>100)
              ...
              if (encontrado != undefined){ 
                  ...
                  // Digitar limpeza do formulário aqui.
