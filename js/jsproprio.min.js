...
// ------------------------------------ ALTERAÇÕES NO SESSION STORAGE DO BANCO DE ADMS -----------------------------------------\\

function versao(){
      alert ("Ver. 6");
}

function Pegarlistadeadms(){
          versao();
          var listadeadms_temp = sessionStorage.getItem("BANCO_ADMS"); // Recupera os dados armazenados conforme a chave.
          listadeadms_temp = JSON.parse(listadeadms_temp); // Converte string para um objeto.
          if (listadeadms_temp == null){ // Caso não haja conteúdo, se for a primeira vez que a pessoa acessa.
          listadeadms_temp = new Array();
          alert ("Banco de adms não existia, então foi criado.");} 
          alert ("Banco de adms já existia, foi apenas recuperado.");
          return listadeadms_temp;}// Retorna a Array para ser pega pelo script da página, seja ela criada ou não.

    function Salvarlistadeadms(banco){
          sessionStorage.setItem("BANCO_ADMS", JSON.stringify(banco));
          alert ("Banco de adms da página salvo adequadamente.");
          return true;}          

// ------------------------------------------------- LOGÍSTICA DOS ADMINISTRADORES --------------------------------------------\\
   /* function Criaradm(nome,email,senha){ // Função para criar primeiro administrador, sem captura de informações (incompleto).
        this.nome = nome;
        this.email = email;
        this.senha = senha;
    ...
    var master = new Criaradm ("Master","master@gmail.com","master"); // Criação do administrador básico (ok).
    listadeadms.push(master);  // Inserção do usuário Master na lista primária de administradores (ok).    
